...
@interface DownloadFolderFragment ()

@end
NSString* DownloadFolderFragment_rootPath;
static PCFileHelper* pCFileHelper;
static NSMutableArray<NSDictionary<NSString*,NSObject*>*>* downloadFileData;
@implementation DownloadFolderFragment

- (void) viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    // Do any additional setup after loading the view.
    if ((DownloadFolderFragment_rootPath!= nil) && [MyUIApplication getselectedPCOnline]){
        [PCFileHelper setNowFilePath:DownloadFolderFragment_rootPath];
    }else {
...
- (void) initData{
    downloadFileData = (downloadFileData==nil)?[[NSMutableArray alloc] init]:downloadFileData;
}
- (void)initView{
    ...
    _bar02CutBtn.layer.shadowColor = [[UIColor blackColor] CGColor];
    _bar02CutBtn.layer.shadowOffset = CGSizeMake(0,0);//偏移距离
    _bar02CutBtn.layer.shadowRadius = 2;//半径
    _bar02CutBtn.layer.shadowOpacity = 0.25;
    _bar02CopyBtn.layer.shadowColor = [[UIColor blackColor] CGColor];
    _bar02CopyBtn.layer.shadowOffset = CGSizeMake(0,0);//偏移距离
    _bar02CopyBtn.layer.shadowRadius = 2;//半径
    _bar02CopyBtn.layer.shadowOpacity = 0.25;
...
- (IBAction)onClick:(id)sender {
    if(sender == _ccopyAddFolderBtn || sender == _cutAddFolderBtn){
            [self addFolderDialog];
    }
    else if (sender == _bar01RefreshBtn || sender == _page04DiskContentLV ){
            [_LoadingDialog showPromptViewOnView:self.view];
            [pCFileHelper loadFiles];
    }
    else if(sender == _bar02CopyBtn){
        _isCheckBoxIn = NO;
    ...
    else if(sender == _bar02DeleteBtn){
        _isCheckBoxIn = YES;
        NSMutableArray<fileBean*>* selectedFolderList = [PCFileHelper getSelectedFolders];
        NSMutableArray<fileBean*>* selectedFileList = [PCFileHelper getSelectedFiles];
        [selectedFolderList removeAllObjects];
    ...
    else if(sender == _bar02SelectAllBtn){
        NSMutableArray<fileBean*>* selectedFolderList = [PCFileHelper getSelectedFolders];
        NSMutableArray<fileBean*>* selectedFileList = [PCFileHelper getSelectedFiles];
        [selectedFolderList removeAllObjects];
        [selectedFileList removeAllObjects];
        ...
        if([_bar02TxSelectAllTV.text isEqualToString:@"全选"]){
            ...
            _bar02CopyBtn.userInteractionEnabled = YES;
            _bar02CutBtn.userInteractionEnabled = YES;
            _bar02DeleteBtn.userInteractionEnabled = YES;
            ...
            _bar02CopyBtn.userInteractionEnabled = NO;
            _bar02CutBtn.userInteractionEnabled = NO;
            _bar02DeleteBtn.userInteractionEnabled = NO;
        ...
        if(selectedFileList.count + selectedFolderList.count == 1) {
            _bar02MoreRenameBtn.userInteractionEnabled = YES;
            [_bar02MoreRenameBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
            if(selectedFileList.count == 1) {
                _bar02MoreSaveBtn.userInteractionEnabled = YES;
                [_bar02MoreSaveBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
                if(!([Login_userId isEqualToString:@""])) {
                    _bar02MoreShareBtn.userInteractionEnabled = YES;
                    [_bar02MoreShareBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
                } else {
                    _bar02MoreShareBtn.userInteractionEnabled = NO;
                    [_bar02MoreShareBtn setTitleColor:[UIColor colorWithRed:211/255.0 green:211/255.0  blue:211/255.0  alpha:1] forState:UIControlStateNormal];
                }
                ...
                _bar02MoreSaveBtn.userInteractionEnabled = NO;
                [_bar02MoreSaveBtn setTitleColor:[UIColor colorWithRed:211/255.0 green:211/255.0  blue:211/255.0  alpha:1] forState:UIControlStateNormal];
                if(!([Login_userId isEqualToString:@""])) {
                    _bar02MoreShareBtn.userInteractionEnabled = NO;
                    [_bar02MoreShareBtn setTitleColor:[UIColor colorWithRed:211/255.0 green:211/255.0  blue:211/255.0  alpha:1] forState:UIControlStateNormal];
                } else {
            ...
            _bar02MoreRenameBtn.userInteractionEnabled = NO;
            [_bar02MoreRenameBtn setTitleColor:[UIColor colorWithRed:211/255.0 green:211/255.0  blue:211/255.0  alpha:1] forState:UIControlStateNormal];
            _bar02MoreSaveBtn.userInteractionEnabled = NO;
            [_bar02MoreSaveBtn setTitleColor:[UIColor colorWithRed:211/255.0 green:211/255.0  blue:211/255.0  alpha:1] forState:UIControlStateNormal];
    ...
    else if(sender == _ccopyCancelBtn){
        [PCFileHelper setIsCopy:NO];
        [PCFileHelper setIsCut:NO];
    ...
    else if(sender == _bar02MoreRenameBtn){
        [self reNameDialog];
    }
    else if(sender == _bar02MoreShareBtn){
        _page04DiskContentBar02MoreRootLL.hidden = YES;
        NSString* name = [PCFileHelper getSelectedFiles][0].name;
        if (name.length > 100){
    ...
    else if(sender == _bar02MoreSaveBtn){
        //...
        NSMutableArray<fileBean*>* selectedFileList = [PCFileHelper getSelectedFiles];
        [selectedFileList removeAllObjects];
        for (fileBean* file in [PCFileHelper getDatas]) {
...
- (void) openFolderSuccess:(NSDictionary*)msg {
    dispatch_async(dispatch_get_main_queue(), ^{
        if(_page04DiskContentActionBar02LL.hidden == NO) {
            ...
            _bar02CutBtn.userInteractionEnabled = NO;
            _bar02DeleteBtn.userInteractionEnabled = NO;
            _bar02SelectAllBtn.userInteractionEnabled = YES;
            [_bar02IconCopyIV setImage:[UIImage imageNamed:@"copy_pressed.png"]];
            [_bar02IconCutIV setImage:[UIImage imageNamed:@"cut_pressed.png"]];
            [_bar02IconDeleteIV setImage:[UIImage imageNamed:@"delete_pressed.png"]];
            [_bar02IconSelectAllIV setImage:[UIImage imageNamed:@"selectedall_normal.png"]];
...
- (void) openFolderFail:(NSDictionary*)msg {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if(_page04DiskContentActionBar02LL.hidden == NO) {
            ...
            _bar02CutBtn.userInteractionEnabled = NO;
            _bar02DeleteBtn.userInteractionEnabled = NO;
            _bar02SelectAllBtn.userInteractionEnabled = NO;
            [_bar02IconCopyIV setImage:[UIImage imageNamed:@"copy_pressed.png"]];
            [_bar02IconCutIV setImage:[UIImage imageNamed:@"cut_pressed.png"]];
            [_bar02IconDeleteIV setImage:[UIImage imageNamed:@"delete_pressed.png"]];
            [_bar02IconSelectAllIV setImage:[UIImage imageNamed:@"selectedall_normal.png"]];
...
- (void) onkeyup:(UIViewController*) vc{
    if(_page04DiskContentBar02MoreRootLL.hidden == NO) {
        ...
        if(_page04DiskContentActionBar02LL.hidden == NO) {
            ...
            if([nowPath isEqualToString:DownloadFolderFragment_rootPath]) {
