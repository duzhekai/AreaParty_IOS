//
//  MKJChatTableViewCell_History.m
//  AreaParty
//
...
@implementation MKJChatTableViewCell_History

- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier])
    {
        self.timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(0,0, [UIScreen mainScreen].bounds.size.width, 25)];
        self.timeLabel.textColor = [UIColor colorWithRed:90/255.0 green:90/255.0 blue:90/255.0 alpha:1];
...
- (void)refreshCell:(MKJChatModel *)model
{
    ...
    if (!model.isRight)
    {
        ...
        self.headImageView.frame = CGRectMake(10,10+25, 40, 40);
        self.backView.frame = CGRectMake(50, 10+25, rec.size.width + 25, rec.size.height + 20);
        image = [UIImage imageNamed:@"chatfrom_bg_normal.9.png"];
    }
    else // 模拟右边
    {
        self.headImageView.frame = CGRectMake(screen_width - 50,10+25, 40, 40);
        self.backView.frame = CGRectMake(screen_width  - 50 - rec.size.width - 25, 10+25, rec.size.width + 25, rec.size.height + 20);
