...
NSString * const TorrentDownloadCacheFolderName = @"areaparty/downloadedfiles/webmanager_download";
...
static NSString * cacheFolder() {
    ...
    dispatch_once(&onceToken, ^{
        if (!cacheFolder) {
            ...
            cacheFolder = [cacheDir stringByAppendingPathComponent:TorrentDownloadCacheFolderName];
        }
        NSError *error = nil;
...
@interface TorrentDownloadReceipt()
...
@end
@implementation TorrentDownloadReceipt
...
@interface DownloadManeger_torrent () <NSURLSessionDataDelegate>
...
@implementation DownloadManeger_torrent
...
- (instancetype)init {
    ...
    return [self initWithSession:session
          downloadPrioritization:TorrentDownloadPrioritizationFIFO
          maximumActiveDownloads:4 ];
}


- (instancetype)initWithSession:(NSURLSession *)session downloadPrioritization:(TorrentDownloadPrioritization)downloadPrioritization maximumActiveDownloads:(NSInteger)maximumActiveDownloads {
    if (self = [super init]) {
...
+ (instancetype)defaultInstance {
    static DownloadManeger_torrent *sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
+ (void)load{
    ...
    NSArray* tempArray = [fileMgr contentsOfDirectoryAtPath:cacheFolder() error:nil];
...
- (TorrentDownloadReceipt *)updateReceiptWithURL:(NSString *)url state:(TorrentDownloadState)state {
    TorrentDownloadReceipt *receipt = [self downloadReceiptForURL:url Name:@""];
    receipt.state = state;
...
- (TorrentDownloadReceipt *)downloadFileWithURL:(NSString *)url
                                       FileName:(NSString*) name
                                  progress:(void (^)(NSProgress * _Nonnull,TorrentDownloadReceipt *receipt))downloadProgressBlock
    ...
    __block TorrentDownloadReceipt *receipt = [self downloadReceiptForURL:url Name:name];
    ...
    dispatch_sync(self.synchronizationQueue, ^{
        ...
        if (receipt.state == TorrentDownloadStateCompleted && receipt.totalBytesWritten == receipt.totalBytesExpectedToWrite) {
            dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (receipt.state == TorrentDownloadStateDownloading && receipt.totalBytesWritten != receipt.totalBytesExpectedToWrite) {
            dispatch_async(dispatch_get_main_queue(), ^{
    ...
    dispatch_sync(self.synchronizationQueue, ^{
        if ([self isActiveRequestCountBelowMaximumLimit]) {
            while (self.queuedTasks.count > 0) {
                ...
                TorrentDownloadReceipt *receipt = [self downloadReceiptForURL:task.taskDescription Name:@""];
                if (task.state == NSURLSessionTaskStateSuspended && receipt.state == TorrentDownloadStateWillResume) {
                    [self startTask:task];
                    break;
                }
            }
...
- (void)startTask:(NSURLSessionDataTask *)task {
    ...
    [self updateReceiptWithURL:task.taskDescription state:TorrentDownloadStateDownloading];
}

- (void)enqueueTask:(NSURLSessionDataTask *)task {
    switch (self.downloadPrioritizaton) {
        case TorrentDownloadPrioritizationFIFO:  //
            [self.queuedTasks addObject:task];
            break;
        case TorrentDownloadPrioritizationLIFO:  //
...
- (TorrentDownloadReceipt *)downloadReceiptForURL:(NSString *)url Name:(NSString*)name{
    ...
    TorrentDownloadReceipt *receipt = self.allDownloadReceipts[url];
    if (receipt) return receipt;
    receipt = [[TorrentDownloadReceipt alloc] initWithURL:url];
    receipt.state = TorrentDownloadStateNone;
    receipt.totalBytesExpectedToWrite = 1;
    receipt.filename = name;
    receipt.truename = name;
...
- (void)resumeWithDownloadReceipt:(TorrentDownloadReceipt *)receipt {
    
    if ([self isActiveRequestCountBelowMaximumLimit]) {
        NSURLSessionDataTask *task = self.tasks[receipt.url];
        // 当请求暂停一段时间后。转态会变化。所有要判断下状态
        if (!task || ((task.state != NSURLSessionTaskStateRunning) && (task.state != NSURLSessionTaskStateSuspended))) {
            [self downloadFileWithURL:receipt.url FileName:@"" progress:receipt.progressBlock destination:nil success:receipt.successBlock failure:receipt.failureBlock];
        }else {
        ...
        receipt.state = TorrentDownloadStateWillResume;
        [self saveReceipts:self.allDownloadReceipts];
...
- (void)suspendAll {
    ...
    for (NSURLSessionDataTask *task in self.queuedTasks) {
        ...
        TorrentDownloadReceipt *receipt = [self downloadReceiptForURL:task.taskDescription Name:@""];
        receipt.state = TorrentDownloadStateFailed;
        [task suspend];
...
- (void)suspendWithDownloadReceipt:(TorrentDownloadReceipt *)receipt {
    
    [self updateReceiptWithURL:receipt.url state:TorrentDownloadStateSuspened];
    NSURLSessionDataTask *task = self.tasks[receipt.url];
    if (task) {
        [task suspend];
        [self safelyDecrementActiveTaskCount];
...
- (void)removeWithDownloadReceipt:(TorrentDownloadReceipt *)receipt {
...
{
    TorrentDownloadReceipt *receipt = [self downloadReceiptForURL:dataTask.taskDescription Name:@""];
    receipt.totalBytesExpectedToWrite = receipt.totalBytesWritten + dataTask.countOfBytesExpectedToReceive;
    receipt.state = TorrentDownloadStateDownloading;
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error
{
    ...
    if (error) {
        ...
        receipt.stream = nil;
        receipt.state = TorrentDownloadStateCompleted;
        dispatch_async(dispatch_get_main_queue(), ^{
            if (receipt.successBlock) {
